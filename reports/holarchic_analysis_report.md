...
**Run:** 2026-09-07T07:24:32.552367+00:00  
**Snapshots:** 346  
...
| H_meso_conv | L2p5 | regional | 0.657 | proxy |
| H_micro_storm | L3 | local | 0.199 | measured |
| H_field_iono | L4 | global | 0.324 | measured |
| H_field_gec | L5 | global | 0.344 | measured |
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=176 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.223 | 176 | weak |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.188 | 175 | negligible |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.372 | 346 | weak |
| H_field_gec→H_field_reso | field_feedback | +0.513 | 346 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 295/346 Snapshots (85%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=295, `none`=22, `electric_to_resonance`=18, `micro_to_electric`=11

## 3. Welche Rückkopplung ist plausibel?

- `H_field_iono→H_field_reso`: r=+0.215 (weak) — Cavity-Hoehe/Leitfaehigkeit moduliert Frequenz und Q
- `H_field_gec→H_field_reso`: r=+0.513 (moderate) — GEC ist die elektrische Architektur der Resonanz
