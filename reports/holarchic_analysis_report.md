...
**Run:** 2026-09-09T17:53:46.164313+00:00  
**Snapshots:** 356  
...
| H_meso_conv | L2p5 | regional | 0.656 | proxy |
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=186 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.255 | 186 | weak |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.183 | 185 | negligible |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.336 | 356 | weak |
| H_field_gec→H_field_reso | field_feedback | +0.505 | 356 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 296/356 Snapshots (83%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=296, `none`=26, `electric_to_resonance`=19, `micro_to_electric`=15

## 3. Welche Rückkopplung ist plausibel?

- `H_field_iono→H_field_reso`: r=+0.230 (weak) — Cavity-Hoehe/Leitfaehigkeit moduliert Frequenz und Q
- `H_field_gec→H_field_reso`: r=+0.505 (moderate) — GEC ist die elektrische Architektur der Resonanz
