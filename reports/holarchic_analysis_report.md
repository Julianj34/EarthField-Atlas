...
**Run:** 2026-09-08T12:57:40.237317+00:00  
**Snapshots:** 351  
...
| H_micro_storm | L3 | local | 0.200 | measured |
...
Makro-Vorbereitung (`H_macro_ocean`) ist der höchste physische Score, Mikro-Aktivierung (`H_micro_storm`) der niedrigste. Die Meso-Skala ist instrumentiert (Wolken-Uebergangsproxy, `confound_type=proxy`); n=181 Snapshots mit Meso-Daten, s. Kopplungstabelle unten.
...
|---|---|---|---|---|
| H_macro_ocean→H_meso_conv | top_down_constraint | -0.240 | 181 | weak |
| H_meso_conv→H_micro_storm | bottom_up_aggregation | -0.182 | 180 | negligible |
| H_micro_storm→H_field_gec | bottom_up_aggregation | +0.351 | 351 | weak |
| H_field_gec→H_field_reso | field_feedback | +0.508 | 351 | moderate |

**Befund:** Der Downstream-Abschnitt (micro→electric→resonance) ist intakt und stark. Der einzige Bruch sitzt bei **macro→micro** und ist in 295/351 Snapshots (84%) die dominante Bruchstelle. Er ist **nicht lokalisierbar**, weil die Meso-Ebene keine Datenquelle hat.

Break-Verteilung über die History: `macro_to_micro`=295, `none`=25, `electric_to_resonance`=19, `micro_to_electric`=12

## 3. Welche Rückkopplung ist plausibel?

- `H_field_iono→H_field_reso`: r=+0.225 (weak) — Cavity-Hoehe/Leitfaehigkeit moduliert Frequenz und Q
- `H_field_gec→H_field_reso`: r=+0.508 (moderate) — GEC ist die elektrische Architektur der Resonanz
